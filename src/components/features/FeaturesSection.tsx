...
export const FeaturesSection = () => {
  ...
  // Derive active feature from scroll position while the section is in view
  useEffect(() => {
    const onScroll = () => {
      if (!sectionRef.current) return;
      const rect = sectionRef.current.getBoundingClientRect();
  ...
  // Intercept wheel inside the section to snap between features
  const handleWheel = (e: React.WheelEvent) => {
    ...
    // Allow normal scroll to exit at bounds
    if (activeFeature === 0 && e.deltaY < 0 && window.scrollY <= currentSnapTop + 2) return;
    if (activeFeature === total - 1 && e.deltaY > 0 && window.scrollY >= currentSnapTop - 2) return;
